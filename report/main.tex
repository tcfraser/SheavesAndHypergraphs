...
\begin{document}
    \title{
    Weighted-Hypergraph Transversals:
    A Practical Approach to Non-Contextuality Inequalities
    }
    ...
    The space of all positive integer multi-sets over the universe of elements $U$ is denoted $\Z_{\geq 0}^{U}$.
    \begin{definition}
        A multi-set $t \in \Z_{\geq 0}^{\s N}$ is a \term{$\w$-super-transversal of $\hypergraph$} if $t \cdot \hypergraph \succeq \w$. A $\w$-transversal $t$ of $\hypergraph$ is \term{minimal} if there does not exist another $\w$-transversal $t'$ of $\hypergraph$ such that $t' \preceq t$. The set of all minimal $\w$-transversals of $\hypergraph$ will be denoted $\transversals{\succeq\w}{\hypergraph}$.
        \[ \transversals{\succeq\w}{\hypergraph} \defined \min\bc{t \in \Z_{\geq 0}^{\s N} \mid t \cdot \hypergraph \succeq \w} \]
    \end{definition}
    Dual to $\w$-super-transversals are \term{$\w$-sub-transversals};


    \[ \transversals{\preceq\w}{\hypergraph} \defined \bc{t \mid t \cdot \hypergraph \preceq \w, \not \exists t' \succeq t : t' \cdot \hypergraph \preceq \w } \]
    \begin{align*}
    t \cdot \hypergraph \preceq \w
    &\iff - t \cdot \hypergraph \succeq -\w  \\
    &\iff k \mathbf{1}_{\nodes} \cdot \hypergraph- t \cdot \hypergraph \succeq k \mathbf{1}_{\nodes} \cdot \hypergraph-\w  \qquad k \defined \max\br{\w}\\
    &\implies k \mathbf{1}_{\nodes} \cdot \hypergraph - t \cdot \hypergraph \succeq k \mathbf{1}_{\nodes} \cdot \hypergraph-\w \qquad \mathbf{1}_{\nodes} \cdot \hypergraph \succeq \mathbf{1}_{\nodes} \cdot \hypergraph \\
    &\implies \br{k \mathbf{1}_{\nodes} - t} \cdot \hypergraph \succeq k \mathbf{1}_{\nodes} \cdot \hypergraph-\w
    \end{align*}
    \begin{align*}
    \ti t \in \transversals{\succeq\br{k \mathbf{1}_{\nodes} \cdot \hypergraph-\w}}{\hypergraph}
    &\implies \ti t \cdot \hypergraph \succeq k \mathbf{1}_{\nodes} \cdot \hypergraph-\w \\
    &\implies k \mathbf{1}_{\nodes} \cdot \hypergraph - \ti t \cdot \hypergraph \preceq \w \\
    &\implies k \mathbf{1}_{\nodes} \cdot \hypergraph - \br{k \mathbf{1}_{\nodes} - t} \cdot \hypergraph \preceq \w \\
    &\implies k \br{\mathbf{1}_{\nodes} \cdot \hypergraph - \mathbf{1}_{\nodes} \cdot \hypergraph} + t \cdot \hypergraph \preceq \w \\
    &\implies t \cdot \hypergraph \preceq \w
    \end{align*}
    If $\ti t$ is minimal, is $t = k \mathbf{1}_{\nodes} - \ti t$? Suppose $t' \succeq t$ was also a transversal,
    \[ t' \cdot \hypergraph \preceq \w \]
    \[ \br{k \mathbf{1}_{\nodes} - \ti t'} \cdot \hypergraph \preceq \w \]
    \[  k \mathbf{1}_{\nodes} \cdot \hypergraph - \ti t' \cdot \hypergraph \preceq \w \]
    ...
    \[ I\br{\ga_1 + \ga_2} = \bc{\br{\ga_{1} + \ga_{2} + \ti \ga_{12}} \cdot \prob{\mscenariowrap} \geq 0 \mid \ti \ga_{12} \in \transversals{\w\br{\ga_1}+\w\br{\ga_2}}{\hypergraph\br{\ga_1 + \ga_2}}} \]
