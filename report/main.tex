...
\begin{document}
    ...
    \begin{definition}
        ...
        A \term{hypergraph} $\hypg$ is defined by a set of nodes $\mathbf{N}\br{\hypg}$ whos $\hypg = \br{\nodes, \edges}$ where $\nodes = \bc{n_1, \ldots, n_p}$ is some finite set and $\edges$ is a finite collection of non-empty subsets $\edges = \bc{\edges_{1}, \ldots, \edges_{q}}$ of $\nodes$. The elements of $\nodes$ are called \term{nodes} while the elements of $\edges$ are called \term{hyperedges} of the hypergraph $\hypg$.
    \end{definition}
    ...
    \begin{definition}
        Given a $\term{weighting}$ $\w \in \Z_{\geq 1}^{\edges}$, a vector $t \in \Z_{\geq 0}^{\nodes}$\footnote{Here, $\Z_{\geq c}^{m}$ denotes the set of all $m$-length integer vectors $x$ such that $x_{i} \geq c, \forall i$.} is an \term{$\w$-transversal} of the hypergraph $\hypg$ if it is a solution to $t \cdot \hypg \succeq \w$.
    \end{definition}

    \begin{definition}
        An $\w$-transversal $t$ of the hypergraph $\hypg$ is \term{minimal} if there does not exist another $\w$-transversal $t'$ of $\hypg$ different from $t$ such that $t' \preceq t$.
    \end{definition}

    In the special case where $\w = \mathbf{1}_{\edges}$, one recovers the notion of a \textit{transversal} of a hypergraph; $t \cdot \hypg \succeq \mathbf{1}_{\edges}$ if and only if the support $\si\br{t} \subseteq \nodes$ of $t$ intersects every hyperedge of $\hypg$, i.e. $\si\br{t} \cap \edges_{i} \neq 0, \forall \edges_i \in \edges$.

    \begin{definition}
        The set of all minimal, $\w$-transversals of $\hypg$ will be denoted $\transversals{\succeq\w}{\hypg}$.
    \end{definition}

    The purpose of \cref{sec:algorithm_section} is to provide an algorithm which, when given a weighting $\w$ and hypergraph $\hypg$, computes $\transversals{\succeq\w}{\hypg}$. In the special case of $\w = \mathbf{1}_{\edges}$, it is customary to identify a class of hypergraphs known as \textit{simple} or \textit{Sperner} hypergraphs wherein $\forall i, j : \edges_i \subseteq \edges_j \Rightarrow j = i$ \cite{Berge_1984}. Non-simple hypergraphs possess a redundancy: whenever $\edges_i \subset \edges_j$, any $t \subseteq \nodes$ which satisfies $t \cap \edges_i \neq 0$ automatically satisfies $t \cap \edges_j \neq 0$ and hence any such $\edges_j$ can be ignored by any transversal generation algorithm. Analogous redundancies are absent from the $\w$-transversal generation problem because $\w \in \Z_{\geq 1}^{\edges}$ is arbitrary.
    ...
    % \[ I\br{\ga_1 + \ga_2} = \bc{\br{\ga_{1} + \ga_{2} + \ti \ga_{12}} \cdot \prob{\mscenariowrap} \geq 0 \mid \ti \ga_{12} \in \transversals{\w\br{\ga_1}+\w\br{\ga_2}}{\hypg\br{\ga_1 + \ga_2}}} \]
