...
\begin{document}
    ...
    \begin{definition}
        A \term{weighted hypergraph} is an ordered pair $\s W = \br{\s H, \w}$ consisting of a hypergraph $\s H$ and corresponding \textit{weighting} $\w \in \Z_{\geqslant 1}^{\s E}$ over the hyperedges of $\s H$.
    \end{definition}
    ...
    \begin{algorithm}[htp]
        ...
        \Procedure{\kSolve{$t$, $k$}}{}
        \Input{A minimal transversal $t \in \mathscr{T}^{*}_{\succcurlyeq}\br{\s W_{k-1}^{k-1}}$.}
        \Output{The set $\mathscr{T}^{*}_{\succcurlyeq}\br{\s W_{k}^{k}} \cap \br{\bigcup_{f \in \s O\br{t}}\Ga^{-1}_{f}\br{\s W^{k}_{k-1}}}$ of minimal transversals of $\s W_{k}^{k}$ which are canonically minimal to some offspring of $t$.}
        \For{$f \in \s O\br{t}$}{
            $\Delta \w = \w_{\s E_{k}} - \sum_{X \in \s X_{k}^{\be,\ga_2}} f_{X}$ \\
            \uIf{$\Delta \w \leqslant 0$}{
                \Yield{$f$}
            }
            \Else{
                \For{$s \in \s S\br{f,\Delta \w}$}{
                    \If{$f+s \in \mathscr{T}^{*}_{\succcurlyeq}\br{\s W_{k}^{k}}$}{
                        \If{$\Ga_{f + s}\br{\s W^{k}_{k-1}} = f$}{
                            \Yield{$f + s$}
                        }
                    }
                }
    ...
    \end{algorithm}
    Here $\s W^{k}_{k-1}$ denotes the partial hypergraph $\s W_{k-1}$ expressed in terms of the generalized nodes $\s X_{k}$.
    \begin{algorithm}[htp]
        \caption{Generation of Weighted Hypergraph Transversals}
        \SetKwFunction{WKS}{WKS}
        \SetKwFunction{WKSWorker}{WKSWorker}
        \label[algorithm]{alg:WKS}
        \Procedure{\WKS{$\s W$}}{}
        \Input{A weighted hypergraph $\s W$.}
        \Output{The set $\mathscr{T}^{*}_{\succcurlyeq}\br{\s W}$ of all minimal transversals of $\s W$.}
        Compute $\bc{\s X_{1}, \ldots, \s X_{q}}$. \\
        Compute $\bc{\s W_{1}^{1}, \s W_{1}^{2}, \s W_{2}^{2}, \ldots, \s W_{q-1}^{q-1}, \s W_{q-1}^{q}, \s W_{q}^{q}}$. \\
        Compute initial transversal $t$: $t_{\s E_1} = \w_{\s E_{1}}$. ($\mathscr{T}_{\succcurlyeq}\br{\s W^{g}_{1}} = \bc{t}$)\\
        \For{$t' \in$ \WKSWorker{$t$, $2$}}{
            \YieldFrom{$U_{\!\s X_{q}}\br{t'}$}
        }
    \end{algorithm}
    \begin{algorithm}[htp]
        \caption{Depth-First Subroutine for \cref{alg:WKS}.}
        \Procedure{\WKSWorker{$t, k$}}{}
        \Input{A minimal transversal $t \in \mathscr{T}^{*}_{\succcurlyeq}\br{\s W_{k-1}^{k-1}}$.}
        \Output{The set of all minimal transversals of $\s W_{q}^{q}$ canonically minimal to $t$ expressed in terms of $\s X_{q}$.}
        \uIf{$k > q$}{
            \Yield{$t$}
        }
        \Else{
            \For{$t' \in$ \kSolve{$t$, $\s W_{k}^{g}$}}{
                \YieldFrom{\WKSWorker{$t', k+1$}}
            }
        }
    \end{algorithm}

    \begin{definition}
        Given a set of generalized nodes $\s X$ for $\s N$, define the \term{unpacking operation} $U_{\!\s X}$ which takes any vector $t \in \Z_{\geq 0}^{\s X}$ to a family of vectors $U_{\!\s X}\br{t} \subset \Z_{\geq 0}^{\s N}$:
        \[ U_{\!\s X}\br{t} \defined \bcm{t' \in \Z_{\geq 0}^{\s N}}{\forall X \in \s X, {\sum}_{n \in X} t'_{n} = t_{X}} \]
    \end{definition}
    \[ \abs{U_{\!\s X}\br{t}} = \prod_{X \in \s X} \binom{t_{X} + \abs{X} - 1}{t_{X}} \]
